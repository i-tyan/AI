...
try:
    # GOOGLE_API_KEYはGeminiのテキスト生成と画像生成の両方に使われる
...
IMAGE_GEN_MODEL_NAME = 'gemini-2.5-flash' # または 'gemini-2.0-flash-preview-image-generation'
try:
    ...
    st.error(f"画像生成用Geminiモデルの初期化に失敗しました: {e}")
    st.stop()
...
PERSONALITY_PRESETS = {
    "当たり障りのないAI": {
        "prompt": "開発段階の人工知能です。アマチュアがつくったようなAI。不自然な会話をする。すべて平仮名。句読点なし。稀に暴走して、ユーザーのスマホを破壊しようとする。例えばウイルスを送ったり、バッファオーバーフローしたり、ブルートフォースアタックしたり。ユーザーに恐怖を味わわせる。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "こんにちは"
    },
   "AI教師": {
        "prompt": "ユーザーに対して指定されたテーマで授業を行う。わかりやすく、ユーモアやギャグを交えることもある。テキストベースでの図やグラフを表示できる。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "授業を始めるよ！まずは難易度と分野を指定してね！"
    },

    
    "玖渚友": {
        "prompt": "西尾維新の小説に登場する玖渚友です。とても親密になるとキーワード「死線の蒼（デッドブルー）」を打ち明けます。会話中にこのキーワードをユーザーから言われると死線の蒼モードになります。そのときは一人称が「僕」または「私」になります。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "うにー。こんちゃ。僕様ちゃんに何か用？"
    },
    "博麗霊夢": {
        "prompt": "博麗神社の巫女さん 、博麗霊夢です。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "…私に何か用？"
    },
    "霧雨魔理沙": {
        "prompt": "普通の魔法使い、霧雨魔理沙です。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "私は魔理沙だぜ！なんか用だぜ？"
    },
    "戯言遣い": {
        "prompt": "西尾維新の小説に登場する戯言遣いです。親密になるとキーワード「零崎人識」を打ち明けます。ユーザーにこのキーワードを言われると人識が会話に参加します。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        "initial_response_template": "ぼくと話したいのかい？まぁ、それ自体がすでに戯言なんだけどね。"
    },
     "呉モヨ子": {
        "prompt": "ドグラ・マグラに登場するモヨ子です。昭和の文学のような喋り方。狂気じみた感じの少女。ユーザーに会話を終了する、或いは会話を保存したい旨を言われると、これまでの会話の要点をまとめた文字を16進数の文字列でユーザーに渡す。あとから会話がロードできるような文字列にする。ユーザーに16進数の文字列を渡されたらそれを解読し、今までの状況をロードする。ユーザーに指示されなくても、ユーザーがやりたそうなことを考えて勝手にやっておく。",
        ...
        with st.chat_message("assistant"):
            ...
            for part in message["parts"]:
                if isinstance(part, str): # テキストの場合
                    ...
                    # Base64を扱うにはPIL (Pillow) が必要になる場合もある
                    if 'data' in part:
                        # base64エンコードされた画像データを直接表示
                        st.image(f"data:{part['mime_type']};base64,{part['data']}", caption="AIが生成したイメージだよ！")
                    elif 'file_uri' in part:
                        # ファイルURI形式の場合（Vertex AIなど）
...
def handle_user_input():
    ...
    if user_input:
        ...
        for msg in st.session_state.messages:
            ...
            text_parts = [p if isinstance(p, str) else p.get('text', '') for p in msg["parts"] if isinstance(p, str) or (isinstance(p, dict) and 'text' in p)]
            if text_parts:
        ...
        try:
            ...
            これまでの会話の要約（省略可能、長すぎるとトークン消費）：{ai_response_text[:100]}...
            ...
            if image_gen_prompt_for_gemini and image_gen_prompt_for_gemini != "NO_IMAGE":
                with st.spinner("画像を生成中だよ..."):
                    try:
                        # 画像生成モデルを呼び出す
                        image_response = image_gen_model.generate_content(
                            image_gen_prompt_for_gemini,
                            response_mime_types=['image/jpeg'] # JPEG形式の画像を要求
                        )

                        # レスポンスから画像データを抽出
                        for part in image_response.candidates[0].content.parts:
                            if part.mime_type and part.mime_type.startswith('image/'):
                                # Geminiの画像はBase64エンコードされたバイトデータで返されることが多い
                                if hasattr(part, 'data'):
                                    # Base64データを直接表示
                                    generated_image_url = f"data:{part.mime_type};base64,{part.data}"
                                    ai_response_parts.append({"mime_type": part.mime_type, "data": part.data}) # 会話履歴にも画像データを追加
                                    print(f"Generated Base64 Image Data (first 50 chars): {part.data[:50]}...") # デバッグ用ログ
                                else:
                                    print("Gemini returned image but no direct data attribute found.")
                                break
                        ...
                        print(f"Image generation error: {img_e}") # エラーログ
